...
func setupTest(t *testing.T) {
	...
	t.Cleanup(func() {
		fmt.Println("cleaning up", runName)
		ctx := context.Background()
		f := make(client.Filters).Add("label", fmt.Sprintf("com.github.alex-brook.composefork.project=%s", runName))
		var errs []error
		...
		imgF := make(client.Filters).Add("label", fmt.Sprintf("com.docker.compose.project=%s", runName))
